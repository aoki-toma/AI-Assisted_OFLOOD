#!/usr/bin/env bash
# File: miyabi_pbs.sh
# Description: HPC adapter for PBS Professional (Miyabi cluster)
# shellcheck disable=SC2034
# The Miyabi cluster, co-operated by the University of Tokyo and the University of Tsukuba
...
setup_env_md() {
  ...
  module load gromacs-gpu/2025.1
}
...
submit_md_job() {
  ...
  local _jid
  _jid=$(
    qsub <<EOF | grep -oE '[0-9]+'
#!/usr/bin/env bash
#PBS -W group_list=${HPC_GROUP}
#PBS -q ${HPC_QUEUE}
#PBS -l select=1:mpiprocs=1:ompthreads=${HPC_NCPUS}
#PBS -l walltime=${WALLTIME_MD}
